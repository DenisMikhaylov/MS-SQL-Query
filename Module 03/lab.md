...
```
Упражнение 4: Написание запросов, использующих внешние соединения
```
Задача 1: Написание оператора SELECT, использующего внешнее соединение
1. В обозревателе решений дважды щелкните запрос 81 - Lab Exercise 4.sql.
2. В окне запроса выделите оператор USE TSQL; и нажмите кнопку Execute.
3. На панели запросов после описания задачи 1 введите следующий запрос:
SELECT
c.custid, c.contactname, o.orderid
FROM Sales.Customers AS c
LEFT OUTER JOIN Sales.Orders AS o ON c.custid = o.custid;
4. Выделите написанный запрос и нажмите кнопку Execute.
5. Проверьте результат. Обратите внимание, что в строках custid 22 и custid 57 отсутствует значение в столбце orderid. Это связано с тем, что в таблице Sales.Orders нет строк для этих двух значений столбца custid. В бизнес-терминах это означает, что в настоящее время нет заказов для
этих двух клиентов.
Результаты: После этого упражнения у вас должно быть базовое понимание того, как писать операторы T-SQL, которые
используют внешние соединения.
```
Упражнение 5: Написание запросов, которые используют перекрестные соединения
```
Задача 1: Выполнение оператора T-SQL
1. В обозревателе решений дважды щелкните запрос 91 - Lab Exercise 5.sql.
2. В окне запроса выделите оператор USE TSQL; и нажмите кнопку Execute.
3. Под описанием задачи 1 выделите код T-SQL и нажмите кнопку Execute. Не волнуйтесь, если вы не понимаете предоставленный код T-SQL, так как он используется здесь для предоставления более реалистичного примера для перекрестного соединения в следующей задаче.

 Задача 2: Напишите оператор SELECT, который использует перекрестное соединение
...
2. Выделите написанный запрос и нажмите «Выполнить».
3. Обратите внимание, что запрос извлек 3294 строки, а в таблице HR.Employees содержится девять строк.
Поскольку перекрестное соединение создает декартово произведение обоих входов, это означает, что в таблице HR.Calendar содержится 366
(3294/9) строк.

Задача 3: Удалить таблицу HR.Calendar
Под описанием задачи 3 выделите написанный запрос и нажмите «Выполнить»
```
